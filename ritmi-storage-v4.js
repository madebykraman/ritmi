...
async function clearProfile(db,s,profileId){const os=db.transaction(s,'readwrite').objectStore(s);const rows=await req(os.getAll());for(const row of rows)if(row.profileId===profileId)await req(os.delete(row.id))}
async function writeSnapshot(db,D){
 ...
 const active=D.activeProfile||fallback;
 for(const profileId of profiles){
  if(profileId===active){await clearProfile(db,'periods',profileId);await clearProfile(db,'logs',profileId);await clearProfile(db,'notes',profileId);await clearProfile(db,'customSignals',profileId);await clearProfile(db,'reminders',profileId);}
...
async function profileSnapshot(db,id){const [meta,profiles,periods,logs,notes,settings,signals,reminders,drafts]=await Promise.all(STORES.map(s=>all(db,s)));const m=meta.find(x=>x.id==='app')||{},D=fresh();D.profiles=profiles.length?profiles:D.profiles;D.activeProfile=id;D.settings=settings.find(x=>x.id==='global')?.value||D.settings;D.periods=periods.filter(x=>x.profileId===id).map(x=>({...x}));D.logs=Object.fromEntries(logs.filter(x=>x.profileId===id).map(x=>[x.date,x.value||{}]));D.notes=Object.fromEntries(notes.filter(x=>x.profileId===id).map(x=>[x.date,x.value||'']));D.customSignals=signals.filter(x=>x.profileId===id).map(x=>({id:x.signalId||x.id,name:x.name}));D.reminders=reminders.filter(x=>x.profileId===id).map(x=>x.value||x);D.communityDrafts=drafts.map(x=>x.value||x);D.onboarded=!!m.onboarded;return D}
...
async function main(){if(!window.indexedDB)throw Error('IndexedDB unavailable');const db=await open();await migrateLegacy(db);const D=await snapshot(db);installCompatibility(db,D);await window.RitmiStorage.persistent();window.RitmiStorage.ready=true;window.dispatchEvent(new Event('ritmi-storage-ready'))}
